...
class SVMRNN:
    ...
    def network_init(self):
        ...
        y_music_src = y_dense_music_src / (
                    y_dense_music_src + y_dense_voice_src + np.finfo(float).eps) * self.x_mixed_src
        y_voice_src = y_dense_voice_src / (
                    y_dense_music_src + y_dense_voice_src + np.finfo(float).eps) * self.x_mixed_src
    ...
    def train(self, x_mixed_src, y_music_src, y_voice_src, learning_rate, dropout_rate):
        ...
        _, train_loss = self.sess.run([self.optimizer, self.loss],
                                      feed_dict={self.x_mixed_src: x_mixed_src, self.y_music_src: y_music_src,
    ...
    def validate(self, x_mixed_src, y_music_src, y_voice_src, dropout_rate):
        y_music_src_pred, y_voice_src_pred, validate_loss = self.sess.run(
            [self.y_pred_music_src, self.y_pred_voice_src, self.loss],
