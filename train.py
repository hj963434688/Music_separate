...
def parse_arguments(argv):
    ...
    parser.add_argument('--dataset_train_dir', type=str, help='训练数据路径', default='./dataset/MIR-1K/Wavfile')
    parser.add_argument('--dataset_validate_dir', type=str, help='验证数据路径',
                        default='./dataset/MIR-1K/UndividedWavfile')
    parser.add_argument('--model_dir', type=str, help='模型保存路径', default='./logs/')
...
def main(args):
    ...
    if not os.path.exists(args.dataset_train_dir) or not os.path.exists(args.dataset_validate_dir):
        ...
        # 1. 导入需要训练的数据集文件路径，存到列表中即可
    train_file_slist = load_file(args.dataset_train_dir)
    ...
    wavs_mono_train, wavs_music_train, wavs_voice_train = load_wavs(filenames=train_file_slist, sr=mir1k_sr)
    # 通过短时傅里叶变换将声音转到频域
